...
MCP endpoints use `protocol: mcp`. The proxy parses sandbox-to-server MCP Streamable HTTP request bodies, validates known MCP request and notification params, evaluates the MCP method against rule `method`, and can match tool calls with the `tool` alias. Generic JSON-RPC endpoints use `protocol: json-rpc`, evaluate `method`, and can match object params.
