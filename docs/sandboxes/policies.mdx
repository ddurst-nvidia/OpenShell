...
5. Edit the YAML: add or adjust `network_policies` entries, binaries, `access`, `rules`, or protocol-specific matchers such as GraphQL operation fields, MCP `mcp_method` / `tool` rules, and generic JSON-RPC `rpc_method` / `params` rules.
...
MCP endpoints use `protocol: mcp`. The proxy parses sandbox-to-server MCP Streamable HTTP request bodies, validates known MCP request and notification params, evaluates the MCP method against `mcp_method`, and can match tool calls with the `tool` alias.
