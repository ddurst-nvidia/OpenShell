...
| `params` | map | No | Params matchers keyed by flattened object-param path. Use dot-separated keys for nested object params, such as `arguments.repository`. Matcher value can be a glob string or an object with `any`. Requests with literal `.` characters in params object keys are rejected before policy evaluation. |
