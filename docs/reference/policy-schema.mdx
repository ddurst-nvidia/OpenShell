...
| `mcp` | object | No | MCP endpoint options. For `protocol: mcp`, `mcp.max_body_bytes` sets the maximum MCP JSON-RPC-over-HTTP request body bytes buffered for inspection. Defaults to `65536`. `mcp.strict_tool_names` defaults to `true` and requires `tools/call` `params.name` values to match `^[A-Za-z0-9_.-]{1,128}$` before policy evaluation. Set it to `false` only for compatibility with MCP servers that intentionally use non-recommended tool names. `mcp.allow_all_known_mcp_methods` defaults to `true`; set it to `false` to require explicit MCP method rules. MCP endpoints must set a concrete `host` and `port` or `ports`; `protocol: mcp` alone is invalid and is not treated as a wildcard endpoint. |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `method` | string | No | MCP method name or OpenShell glob, such as `initialize`, `tools/list`, `tools/call`, or `tools/*`. Required when `mcp.allow_all_known_mcp_methods` is `false`; otherwise omitted method uses the endpoint method profile. `*` is OpenShell policy matching syntax, not JSON-RPC method syntax. |
